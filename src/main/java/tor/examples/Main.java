...
//        };
        ...
        OnionRouter southsea = Consensus.getConsensus().getRouterByName("southsea0");
        TorSocket sock = new TorSocket(southsea);

        final String ONION = "3g2upl4pq6kufc4m";
        TorCircuit hscirc = HiddenService.establishHSCircuit(sock, ONION);



 //
 // circ.createRouteors[0].");      circ.createRoute("southsea0");
 //
 // circ.createRouteors[0].");      circ.waitForState(TorCircuit.STATES.READY);
 //
 // circ.createRouteors[0].");      circ.send(null, TorCircuit.RELAY_DROP, false, (short)0);
//        circ.destroy();
//
//
// circ.destroy();
